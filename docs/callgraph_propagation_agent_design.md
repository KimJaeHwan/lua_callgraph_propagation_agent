...
이 문서는 프로젝트 scaffold용 설계 초안이다. 현재 입력 스키마와 최소 fixture는 고정했고, 다음 단계는 vanilla feature JSON에서 SQLite reference call graph를 생성하는 변환 스크립트와 deterministic propagation scorer를 구현하는 것이다.
