#!/usr/bin/env python3
"""
Unified config loader for the lua_callgraph_propagation_agent runtime.

Supported config styles:

1. Minimal user-facing format
   - session_name
   - user_input{} or target{}
   - analysis{}
   - graph_config{}
   - tooling{}
...
Callers should always use resolve_paths(); it expands derived locations such as
reference DBs, retrieval indexes, result JSONs, manual force anchor JSON, and
query-feature manifest paths.
"""
...
def load_config(path: str | Path) -> dict[str, Any]:
    """Load a pipeline config and attach internal path metadata."""
...
def is_new_format(config: dict[str, Any]) -> bool:
    """Return True for any non-legacy config layout."""
    return "paths" not in config


def resolve_paths(config: dict[str, Any]) -> dict[str, Any]:
    """Return the full runtime path map for *config*."""
...
def deferred_top_candidates(config: dict[str, Any]) -> int:
